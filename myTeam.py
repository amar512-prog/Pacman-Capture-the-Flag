...
class DummyAgent(CaptureAgent):
    ...
    def registerInitialState(self, gameState):
        ...
        for i, val in enumerate(possible_Position_List):
            if i in self.getOpponents(gameState):
                possible_Position_List[i][gameState.getInitialAgentPosition(i)] = 1.0
        ...
        if ini_num == 0:
            if strategy[self.index] == 1:
                if len(targetFood[self.index]) > 1:
                    action = self.aStarSearch(gameState, targetFood[self.index][0])
                    ...
                    for i in self.getOpponents(gameState):
                        if gameState.getAgentState(i).isPacman:
                            if gameState.getAgentState(i).getPosition() == None:
                                ...
                                pacmanPosition = gameState.getAgentState(i).getPosition()
                            action = self.aStarSearch(gameState, pacmanPosition)
        return action
    ...
    def attackfeatures(self, gameState, pos):
        ...
        if 0 < ghostDist < 6:
            ...
            if (pos) in targetFood[self.index]:
                features['fd'] = 1.00
            notScaredGhosts = [a for a in enemies if not a.isPacman and a.scaredTimer == 0]
            for capsule in capsuleList:
                if capsule == (pos):
                    if ghostDist > 0:
            ...
            for x in range(0, self.width):
                for y in range(0, self.height):
                    if not self.datalayout.walls[x][y]:
                        manhattanDistance = abs(self.mypos[0] - x) + abs(self.mypos[1] - y)
    ...
    def evaluate(self, gameState, pos):#Computes a linear combination of features and feature weights
    ...
    def getdiscountrewards(self, gameState, position):#computes the next state rewards when execute action
        x, y = position
        new_reward = {}
            ...
            if gameState.getAgentPosition(i) != None:
                ...
                for p in tem_pos:
                    if not gameState.hasWall(p[0], p[1]) and tem_pos[p] > 0:
                        _, _, next_action = self.getdiscountrewards(gameState, p)
    ...
    def choose_strategy(self, gameState):# choose the strategy
        ...
        self.border.sort(key=self.getDistance)
        my_carry = gameState.getAgentState(self.index).numCarrying + gameState.getAgentState(other_index).numCarrying
        enemy_carry = sum([gameState.getAgentState(i).numCarrying for i in self.getOpponents(gameState)])
    ...
    def get_position_pro(self, gameState):#assign the inital rewards for each position
    ...
    def choose_enemy_possible_position(self, gameState):# find the enemy position with max probablity position
        for i in self.getOpponents(gameState):
